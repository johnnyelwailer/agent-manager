...
**Design principle: it's all about the illusion of speed.**

The user should feel like the system already knows what they want. Every
mechanism — scouts, conflict detection, context sharing — exists to make the
gap between "Submit" and "done" feel like it doesn't exist. The system does
real work, but the user shouldn't see it.

**Q1: How do merge conflicts between scout diffs get resolved?**

Structural conflict check first. Then the merge agent either rubber-stamps
clean patches or reasons through conflicts — but only the conflicts.

When the user hits Submit, before the merge agent starts, the host classifies
all scout diffs by scanning JSON Patch paths (O(n), instant):

```typescript
interface PatchClassification {
  clean: ScoutResult[];          // no path overlaps with any other diff or user edit
  conflicting: ScoutResult[][];  // groups whose paths overlap
  pending: PlanComment[];        // scouts still working — no diff yet
}
```

Two diffs "conflict" if any patch paths share a common prefix (both touch
`/blocks/2/payload/nodes/...`). This is a string prefix check, not semantic
analysis — fast enough to run synchronously before the agent starts.

The merge agent gets a structured prompt that lets it skip the easy work:

```
CLEAN — approve or override:
  Comment 1 → diff: <patch>  "Added MFA node between Login and Dashboard"
  Comment 3 → diff: <patch>  "Added OWASP checklist to Security Requirements"
  Respond APPROVE to apply as-is, or provide a replacement.

CONFLICTS — must resolve:
  Comments 2 + 4 both modify block "User Journey":
    Scout 2: <patch>  "Rerouted auth flow through SSO"
    Scout 4: <patch>  "Added rate limiting after auth"
    Produce a single merged result for this block.

NO RESULT — handle from scratch:
  Comment 5: "make this more concise" (scout still working)
```

The agent approves clean patches in one token each. It only spends real
thinking time on conflicts and unfinished scouts. For a typical batch of 6
comments where 4 are clean, the agent does a few seconds of work instead of
regenerating the entire plan.

**Q2: How to make merging feel fast but still be thorough?**

Layer the responses so the user sees progress immediately:

```
  t=0ms     User hits Submit
             Host classifies patches (instant, structural)

  t=30ms    Clean scout diffs applied to UI
             Blocks update. Comments get checkmarks.
             User sees: most of their feedback already applied.

  t=100ms   Merge agent session starts (background)

  t=1-2s    Agent streams APPROVE / APPROVE for clean patches
             Confirms what the user already sees.
             Subtle provisional border fades off each block.

  t=3-5s    Agent streams resolution for conflicts
             Affected blocks swap in with smooth transition.

  t=5-8s    Agent handles remaining unfinished scouts from scratch
             Last blocks fill in.

  t=8s      All done. Plan is final.
```

The user perceives ~2 seconds even though the agent worked for 8. The
clean patches appear at t=30ms — that's the magic moment. Everything after
that is refinement happening in peripheral vision.

The provisional indicator is deliberately subtle: a thin colored left
border, not a modal or badge. It communicates "settling" not "uncertain."
In the common case (independent comments), the agent just confirms
everything and the borders dissolve silently. The user never notices
Phase B happened.

**Degenerate case:** all comments touch the same block (e.g., 5 comments
on the flow graph). Nothing is classified as clean, so nothing appears at
t=30ms. The UI shows a per-block progress indicator: "Resolving 5
comments..." This is honest — it's also the case where instant application
would be wrong. But it's rare. Most review sessions spread comments across
blocks.

**Q3: How to handle context-dependent comments ("here too")?**

No threading. No manual linking. Zero friction.

Every scout already gets the full plan JSON. The fix: also give it **all
pending comments** as read-only context. Comments are tiny (a few hundred
tokens total) compared to the plan (thousands of tokens). The marginal
cost is negligible.

```
ALL PENDING COMMENTS (read-only — do not address these directly):
  1. [flow, node "Login"]  "Replace basic auth with OAuth2"
  2. [flow, node "Signup"] "here too"
  3. [wireframe, element "auth-form"] "needs OAuth buttons"

YOUR TASK — comment 2 only:
  Block: flow block "User Journey"
  Anchor: node "Signup"
  Content: "here too"

Produce the minimal JSON patch that addresses this comment.
```

The scout reads "here too," sees comment 1 in the preamble, connects the
dots, applies OAuth2 to the Signup node. No dependency detection. No user
action. The LLM does what LLMs are good at: resolving references from
surrounding context.

**Why this beats every alternative:**

- **Threading** forces the user to link comments manually. That's friction
  in the middle of a fast review flow. The user is clicking around the plan,
  dropping thoughts as they go. Making them think about which comment links
  to which interrupts the creative flow.

- **Dependency detection** (O(n²) LLM classifier) adds latency before
  any scout starts. For 10 comments, that's 45 pairs to classify. You burn
  time and money before doing any actual work.

- **Shared context preamble** adds ~200 tokens per scout prompt and costs
  nothing in latency. The scout starts immediately. The LLM resolves
  references as a side effect of reading the prompt. No extra step, no extra
  UI, no extra API call. It just works.

**Edge case — genuinely unresolvable comments:**
If a scout can't make sense of a comment even with all other comments as
context (e.g., "same as last time" referring to a previous session, not any
current comment), it produces `status: 'failed'` with a summary explaining
the ambiguity. The merge agent gets one more shot with even richer context
(all comments + all scout results + user edits). If it still can't resolve,
the comment stays open post-submission with a note: "Could you be more
specific?" — the only point where the system asks the user anything, and
it happens *after* all other changes are applied, not during the review.
